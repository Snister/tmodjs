...
module.exports = {
  ...
  compile: function (file, recursion) {
    ...
    if (file) {
      ...
      console.log('总共模板数量：', list.length);
  ...
  _initCache: function () {
    ...
    var walk = function (dir) {
      ...
      // 过滤编译目标目录
      if(!that.filterCompileFolders(dir))
        return;
  ...
  // ignore some folders when compile or watch
  filterCompileFolders: function(folderPath) {
    var obj = this;

    // 排除 'options.exclude_folders' 中配置的目录
    var filterFolder = obj.options.exclude_folders;

    if(filterFolder && _.indexOf(filterFolder, folderPath.replace(this.base, '')) > -1) {
    return false;
    }
  ...
  init: function (input, options) {
    ...
    this.on('newListener', function (event, listener) {
      ...
      if (watch && event === 'watch') {
        ...
        var watchFilter = function (folderPath) {
          // 是否排除目录
          if(!obj.filterCompileFolders(folderPath))
            return;

          var name = path.basename(folderPath);
          // obj.options.debug && console.log(folderPath, !FILTER_RE.test(name) && folderPath !== obj.output);
          // 排除“.”、“_”开头或者非英文命名的子目录
          return !FILTER_RE.test(name) && folderPath !== obj.output;

        }.bind(obj);

        watch(obj.base, watchCallback, watchFilter, fs);
